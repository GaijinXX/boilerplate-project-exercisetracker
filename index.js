...
app.get('/api/users/:_id/logs', async (req, res) => {
  ...
  res.json({username: user.username, count: user.exercises.length, _id: user._id, log: user.exercises.slice(0, req.query.limit).map(el => {el.date = (new Date(el.date)).toDateString(); return el})});
});
